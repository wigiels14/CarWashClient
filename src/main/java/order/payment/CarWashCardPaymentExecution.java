...
public class CarWashCardPaymentExecution implements PaymentExecution {
	...
	public boolean executePayment() {
		...
		double accountBalace = Client.mainCustomerInterfacePanel.topCustomerInterfacePanel.getCustomerAccountBalance();
		if (canExecutePayment(accountBalace, serviceCost)) {
			double newAccountBalace = payment.executePayment(customer.getAccountBalance(), order.getCost());
	...
	public void accountBalanceUpdate(double newAccountBalance) {
		...
		Client.mainCustomerInterfacePanel.topCustomerInterfacePanel.accountBalaceText();
		Customer customer = Client.mainCustomerInterfacePanel.topCustomerInterfacePanel.getCustomer();
		Client.mainCustomerInterfacePanel.changePersonalDataPanel.sendChangeCustomerAccountBalance(customer.getId(),
